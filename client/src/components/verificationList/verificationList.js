  ...
  return (
    <div>
      <div className="introContainer">
        ...
        <p id="secondHeader">
          {t("verificationList.expectingYou")}{" "}
          <b>{localStorage.getItem("appointmentTime")} </b>{" "}
          {t("verificationList.onThe")}{" "}
          <b>{localStorage.getItem("appointmentDate")}</b>{" "}
          {t("verificationList.at")}{" "}
          <b>
            {" "}
            {localStorage.getItem("hospitalLang")}{" "}
            {t("verificationList.hospital")}
          </b>{" "}
        </p>
      </div>

      <h2 id="forgetText">
        <b>{t("verificationList.dontForget")}</b>
      </h2>
      ...
      <Link id="link" to="/dashboard">
        <button className="confirmButton">
          {t("verificationList.confirm")}
        </button>
      </Link>
      {/* ********removed book a taxi until further notice*********8 */}
