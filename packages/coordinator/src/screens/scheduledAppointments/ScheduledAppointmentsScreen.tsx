...
export default function ScheduledAppointmentsScreen({ appointmentsWithDonorDetails, isLoading }: ScheduledAppointmentsScreenProps) {
    ...
    const columns: CardTableColumn<BookedDonationWithDonorDetails>[] = [
        {
            ...
            cellRenderer: ({ donationStartTimeMillis }) => DateUtils.ToDateString(donationStartTimeMillis),
        },
        {
            label: "שעה",
            cellRenderer: ({ donationStartTimeMillis }) => DateUtils.ToTimeString(donationStartTimeMillis),
        },
        {
            label: "בית חולים",
        ...
        {
            label: "שם תורם",
            sortBy: SortingUtils.StringComparator<BookedDonationWithDonorDetails>((d) => d.firstName),
            cellRenderer: ({ firstName }) => firstName,
        },
        {
            label: "שם משפחה תורם",
            sortBy: SortingUtils.StringComparator<BookedDonationWithDonorDetails>((d) => d.lastName),
            cellRenderer: ({ lastName }) => lastName,
        },
        {
            label: "סוג דם",
            sortBy: SortingUtils.StringComparator<BookedDonationWithDonorDetails>((d) => d.bloodType),
