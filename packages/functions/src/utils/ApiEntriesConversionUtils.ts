import { BookingChange, DbAppointment, FunctionsApi } from "@zm-blood-components/common";
...
const getRecentChangeType = ({lastChangeTime, lastChangeType} : DbAppointment) => {
  let recentChangeType : BookingChange | undefined

  if (lastChangeTime && Date.now() - lastChangeTime.toMillis() < 1000 * 60 * 60 * 24) {
